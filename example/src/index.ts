...
// warm-ups
{
  ...
  // verify proof (only if not Bun, because it fails due to some worker error)
  if (!isBun()) {
    if (!(await snarkjs.groth16.verify(vk, publicSignals, proof, USE_SINGLE_THREAD))) {
      throw new Error("Verification failed");
    }
  }

  // generate with ICICLE
  {
    const { proof, publicSignals } = circomkitFFI.icicle_prove(
      circomkit.path.ofCircuitWithInput(warmUpCircuit, warmupInput, "wtns"),
      circomkit.path.ofCircuit(warmUpCircuit, "pkey"),
      "CPU"
    );
